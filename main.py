...
"""
Polymarket Layered Bot — CLEAR BET INSTRUCTIONS (PT-BR)

Goal: Every alert must be crystal clear:
- "APOSTE A FAVOR: <statement>" OR "APOSTE CONTRA: <statement>"
- 1-line main reason
- evidence bullets
- anti-spam + paper trading
...
# Filters (proactive, but not crazy)
...
EDGE_MIN_PP = float(os.getenv("EDGE_MIN_PP", "3.2").strip())  # minimum edge (percentage points)
...
UA = os.getenv("HTTP_UA", "Mozilla/5.0 (compatible; PolymarketLayerBot/1.3)")
...
# =========================
def fetch_markets(limit: int, active_only: bool) -> List[Dict[str, Any]]:
    params = {
        "limit": limit,
        "closed": "false",
    ...
    data = http_get(f"{GAMMA_BASE}/markets", params=params)
    ...
    for m in data:
        q = (m.get("question") or m.get("title") or m.get("name") or "").strip()
        slug = m.get("slug") or ""
...
def score_market(m: Dict[str, Any], d: Dict[str, float]) -> float:
    liq = safe_float(m.get("liquidity"))
    vol_delta = safe_float(d.get("vol_delta"))
    pm = abs(safe_float(d.get("price_move_pct")))
    ...
    return clamp(liq_s + vd_s + pm_s + urg_s, 0.0, 15.0)
...
def pick_candidates(markets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    out.sort(key=lambda x: safe_float(x.get("_score")), reverse=True)
    return out[:CANDIDATES_TOP_N]

# =========================
# INTEL (GDELT pulse)
# =========================
def gdelt_pulse(query: str, hours: int) -> Dict[str, Any]:
    ...
    common = {"query": query, "mode": "ArtList", "format": "json", "maxrecords": 60, "sort": "HybridRel"}
    ...
    try:
        ...
        if isinstance(recent, dict):
            for a in (recent.get("articles") or [])[:4]:
...
# =========================
# CLEAR BET INSTRUCTION
# =========================
def bet_instruction(m: Dict[str, Any], side: str) -> str:
    """
    Crystal clear instruction:
    - APOSTE A FAVOR: <question>  (side YES)
    - APOSTE CONTRA: <question>   (side NO)
    """
    question = (m.get("question") or "").strip()
    if not question:
        question = (m.get("slug") or "este mercado").strip()
    if side == "YES":
        return f"🟢 APOSTE A FAVOR: {question}"
    return f"🔴 APOSTE CONTRA: {question}"

def action_line_from_edge(edge_pp: float) -> str:
    if abs(edge_pp) >= (EDGE_MIN_PP + 1.8):
        return "ENTRAR AGORA (se spread ok)"
    return "OBSERVAR 2–5 min (pode destravar)"

# =========================
# DECISION
# =========================
def estimate_probability(m: Dict[str, Any], intel: Dict[str, Any]) -> (float, float, List[str]):
    reasons: List[str] = []
    p_market = clamp(safe_float(m.get("yes_price")), 0.01, 0.99)

    d = m.get("_deltas") or {}
    vol_delta = safe_float(d.get("vol_delta"))
    pm = safe_float(d.get("price_move_pct"))
    liq = safe_float(m.get("liquidity"))
    ...
    if intel.get("ok"):
        ...
        if rc >= 8 and spike >= 0.25:
            conf += 0.08
            reasons.append(f"News pulse ↑ (últ. {GDELT_WINDOW_H}h: {rc} vs prev: {pc}).")
        elif rc >= 8:
            conf += 0.04
            reasons.append(f"News flow steady (últ. {GDELT_WINDOW_H}h: {rc}).")

        # Directional nudge aligned with momentum sign
        nudge = clamp(spike, -0.5, 1.8) * 0.023
        if pm > 0:
            p += max(0.0, nudge)
            if nudge > 0:
                reasons.append("Momentum YES + atenção subindo (repricing possível).")
        elif pm < 0:
            p -= max(0.0, nudge)
            if nudge > 0:
                reasons.append("Momentum NO + atenção subindo (repricing possível).")

        for a in (intel.get("sample") or [])[:2]:
            title = (a.get("title") or "").strip()
            url = (a.get("url") or "").strip()
            if title and url:
                reasons.append(f"Fonte: {title} — {url}")

    # Micro adjustment from momentum only
    p += clamp(pm / 100.0, -0.025, 0.025) * 0.55

    if abs(pm) >= MIN_PRICE_MOVE_PCT:
        reasons.append(f"Movimento de preço: {pm:+.2f}% (YES).")
    if vol_delta >= MIN_VOL_DELTA:
        reasons.append(f"Δ Volume: +{vol_delta:,.0f} (desde o último check).")

    p = clamp(p, 0.01, 0.99)
    conf = clamp(conf, 0.34, 0.92)
    if not reasons:
        reasons = ["Sinal fraco (pouca evidência externa)."]
...
def maybe_close_old_paper_trades(markets_by_id: Dict[str, Dict[str, Any]]) -> List[str]:
    ...
    for t in db_open_paper_trades():
        age_h = (ts() - safe_int(t["created_at"])) / 3600.0
...
# ALERTS
# =========================
def should_alert(m: Dict[str, Any], dec: Dict[str, Any]) -> bool:
    recent = db_recent_alert(m["id"])
    if not recent:
        return True
    old_edge = safe_float(recent["edge_pp"])
    new_edge = safe_float(dec["edge_pp"])
    return abs(new_edge) >= abs(old_edge) + REALERT_EDGE_BUMP_PP
...
def format_alert(m: Dict[str, Any], dec: Dict[str, Any], reasons: List[str]) -> str:
    ...
    edge_pp = safe_float(dec["edge_pp"])
    p_market_pct = safe_float(dec["p_market"]) * 100.0
    p_model_pct = safe_float(dec["p_model"]) * 100.0
    conf = safe_float(dec["conf"])
    side = dec["side"]

    instruction = bet_instruction(m, side)
    action = action_line_from_edge(edge_pp)

    main_reason = (
        f"O mercado precifica ~{p_market_pct:.1f}%, "
        f"mas o modelo sugere ~{p_model_pct:.1f}% "
    ...
    return (
        ...
        f"🧠 Motivo principal:\n{main_reason}\n\n"
        f"📌 Evidências:\n{rs_txt}\n\n"
        f"📊 Dados: Score={score:.2f} | VolΔ={vol_delta:,.0f} | Move={pm:+.2f}% | Liq={safe_float(m.get('liquidity')):,.0f}\n"
        f"🔗 {m.get('url')}"
    )
...
def format_watchlist(candidates: List[Dict[str, Any]]) -> str:
    ...
    digest = "\n".join(lines) if lines else "- (sem candidatos agora)"
    return (
        "👀 WATCHLIST (sem edge suficiente ainda)\n"
        "Top mercados pra monitorar agora:\n"
        f"{digest}\n\n"
        "Dica: se volume subir de novo OU preço mover mais ~0.8–1.2%, pode virar EDGE."
    )
...
def run_scanner() -> None:
    ...
    if not markets:
        tg_send("⚠️ Bot: nenhum mercado retornou da API. Verifique GAMMA_BASE/conexão.")
        return
    ...
    # Close old paper trades (avoid spam)
    for msg in maybe_close_old_paper_trades(markets_by_id)[:6]:
        tg_send(msg)

    candidates = pick_candidates(markets)
    if not candidates:
        tg_send("🤖 Scanner: nenhum candidato passou filtros agora.")
        return

    sent = 0
    for m in candidates:
        intel_q = keywords_from_market(m)
        intel = gdelt_pulse(intel_q, hours=GDELT_WINDOW_H) if intel_q else {"ok": False}
        ...
        if DRY_RUN == "1":
            db_create_paper_trade(
                ...
                meta={
                    ...
                    "reasons": reasons[:8],
                    "url": m.get("url"),
                    "instruction": bet_instruction(m, dec["side"]),
                }
            )
...
def run_report() -> None:
    ...
    digest = "\n".join(lines) if lines else "- (nenhum alerta nas últimas 24h)"
